...
checked_main(int argc, char **argv)
{
  ...
  spot :: const_tgta_explicit_ptr prop1 = nullptr;
  spot:: const_tgta_explicit_ptr pro = nullptr ;
  spot:: const_tgta_ptr tgta =nullptr ;
  //auto prop= nullptr ;
  //spot::const_twa_ptr pro = nullptr;
  ...
  tm.start("translating formula");
  {
    ...
    ap_set= atomic_prop_collect_as_bdd(f,prop);
    prop1=tgba_to_tgta(prop,ap_set);
    pro= minimize_tgta(prop1);
  }
  ...
  if (output == DotFormula)
    {
      tm.start("dot output");
      spot::print_dot(std::cout, pro->get_ta());
      tm.stop("dot output");

      goto safe_exit;
    }

      product = spot::product(model,tgta);
