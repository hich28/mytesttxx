...
checked_main(int argc, char **argv)
{
  ...
  spot :: tgta_explicit_ptr prop1 = nullptr; 
   //auto product ;
  ...
  tm.start("translating formula");
  {
    ...
    bdd ap_set= atomic_prop_collect_as_bdd(f,prop);
    prop1= spot::tgba_to_tgta(prop , ap_set);
  ...
  if (output == DotFormula)
    {
      ...
      //spot::print_dot(std::cout, prop1);
      spot::print_dot(std::cout, prop1->get_ta());
      tm.stop("dot output");
      goto safe_exit;
    }
    //  product = spot::otf_product(model, prop1);
     product = spot::product(model, prop1);
